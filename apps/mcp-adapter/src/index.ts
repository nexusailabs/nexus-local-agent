...
function createServer(): McpServer {
  ...
  server.registerTool(
    "nexus_exec",
    {
      ...
      inputSchema: z
        .object({
          argv: z.array(z.string()).min(1).describe("Executable followed by arguments."),
          cwd: z.string().min(1).optional().describe("Working directory on the target node."),
          timeout_ms: z
            .number()
            .int()
            .min(1)
  ...
  server.registerTool(
    "nexus_write_file",
    {
      ...
      inputSchema: z
        .object({
          path: z.string().min(1).describe("Path on the target node."),
          content: z.string().max(4_000_000).describe("Complete replacement contents."),
          node_id: nodeId,
        })
        .strict(),
      annotations: {
    ...
    async ({ path, content, node_id }) => {
      try {
        return response(
          await client.executeTool("fs.write", { path, content }, targetOptions(node_id)),
        );
      } catch (error) {
        return failure(error);
      }
  ...
  server.registerTool(
    "nexus_git_status",
    {
      ...
      description: "Return porcelain Git status for a repository on the MBP by default.",
      inputSchema: z
        .object({
          cwd: z.string().min(1).describe("Repository directory on the target node."),
          node_id: nodeId,
        })
        .strict(),
      annotations: {
    ...
    async ({ cwd, node_id }) => {
      try {
        return response(
          await client.executeTool("git.status", { cwd }, targetOptions(node_id)),
        );
      } catch (error) {
        return failure(error);
      }
  ...
  server.registerTool(
    "nexus_git_diff",
    {
      ...
      description: "Return a staged or unstaged Git diff from a repository on the MBP by default.",
      inputSchema: z
        .object({
          cwd: z.string().min(1).describe("Repository directory on the target node."),
          staged: z.boolean().default(false).describe("Return the staged diff."),
          node_id: nodeId,
        })
        .strict(),
      annotations: {
    ...
    async ({ cwd, staged, node_id }) => {
      try {
        return response(
          await client.executeTool("git.diff", { cwd, staged }, targetOptions(node_id)),
        );
      } catch (error) {
        return failure(error);
      }
  ...
  server.registerTool(
    "nexus_read_document",
    {
      ...
      inputSchema: z
        .object({
          path: z.string().min(1),
          max_bytes: z.number().int().min(1_000).max(4_000_000).default(200_000),
          node_id: nodeId,
        })
        .strict(),
      annotations: {
