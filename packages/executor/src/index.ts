    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      const response = await llm.turn({
        system: [
          ...
          "Work silently through tools: do not narrate plans, progress, tool choice, or ordinary retries. Diagnose and retry recoverable failures autonomously.",
          "Prefer recoverable operations. Before an irreversible or external effect, verify that the objective authorizes that exact effect and resolve the exact target without broad paths, unresolved variables, or ambiguous recipients.",
