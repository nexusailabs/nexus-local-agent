...
const nodes: NodeSpec[] = [
  ...
  },
  {
    ...
    platform: 'linux-x64',
    memoryGb: 64,
    capabilities: ['inference', 'exec', 'fs', 'git', 'containers', 'browser', 'ci', 'long-running'],
    reachability: 'lan',
    tags: ['rocm'],
    executionClass: 96,
    reliabilityClass: 90,
  ...
  },
  {
    ...
    platform: 'darwin-arm64',
    memoryGb: 16,
    capabilities: ['exec', 'fs', 'git', 'browser', 'ci', 'network-probe', 'long-running', 'control-standby'],
    reachability: 'wan',
    region: 'hk',
    tags: ['remote'],
    executionClass: 62,
...
describe('heterogeneous routing', () => {
  it('routes planning inference to the heavy M5 model', () => {
    expect(routeInference(nodes, 'plan')).toMatchObject({ nodeId: 'm5', modelId: 'heavy' });
  });
